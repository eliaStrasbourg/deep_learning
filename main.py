...
#random_image = []
random_image = SessionState.get(img=[])
i = 1
while i < 10:
	...
	random_image.img.insert(0, random.choice(images))
	i += 1
pick_img = SessionState.get(img=[])
pick_img.img = st.sidebar.radio("Which image?", [x for x in range(1, len(random_image.img) + 1)])
...
def main():
    with input_col:
        ...
        for x in range(3):
            for y in range(3):
                img = mpimg.imread(random_image.img[y + x * 3])
                ax[x, y].imshow(img)
